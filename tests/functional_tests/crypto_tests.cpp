...
inline std::ostream &operator <<(std::ostream &o, const crypto::public_key &v) { return o << "<" << crypto::pod_to_hex(v) << ">"; }
inline std::ostream &operator <<(std::ostream &o, const crypto::secret_key &v) { return o << "<" << crypto::pod_to_hex(v) << ">"; }
inline std::ostream &operator <<(std::ostream &o, const crypto::key_derivation &v) { return o << "<" << crypto::pod_to_hex(v) << ">"; }
inline std::ostream &operator <<(std::ostream &o, const crypto::key_image &v) { return o << "<" << crypto::pod_to_hex(v) << ">"; }
inline std::ostream &operator <<(std::ostream &o, const crypto::signature &v) { return o << "<" << crypto::pod_to_hex(v) << ">"; }
inline std::ostream &operator <<(std::ostream &o, const crypto::hash &v) { return o << "<" << crypto::pod_to_hex(v) << ">"; }
...
scalar_t scalar_from_str(const std::string& str)
{
  ...
  if (!parse_tpod_from_hex_string(str, sk))
...
{
  crypto::hash hash;
  if (!parse_tpod_from_hex_string(str, hash))
...
#define TEST(test_name_a, test_name_b) \
  ...
  static bool test_name_a ## _ ## test_name_b()
#define ASSERT_TRUE(expr)  CHECK_AND_ASSERT_MES(expr, false, "This is not true: " #expr " at " << LOCATION_SS)
#define ASSERT_FALSE(expr) CHECK_AND_ASSERT_MES((expr) == false, false, "This is not false: " #expr " at " << LOCATION_SS)
...
TEST(crypto, keys)
{
  ...
  bool r = false;
  crypto::public_key pk;
  r = parse_tpod_from_hex_string("1b546af91d31fdb1c476fd62fbb65b6fd5ed47804185fc77d48bc4cc00f47ef0", pk);
  ASSERT_TRUE(r);

  crypto::secret_key sk;
  r = parse_tpod_from_hex_string("407b3b73df8f11737494bdde6ca47a42e1b537390aec2fa781a2d170335c440f", sk);
  ASSERT_TRUE(r);
...
TEST(ml2s, hs)
{
  ...
  scalar_t k = parse_tpod_from_hex_string<scalar_t>("deefd263cbfed62a3711dd133df3ccbd1c4dc4aac21d7405fd667498bf8ebaa1");
  std::cout << k << ENDL;
...
std::string ml2s_sig_to_python(const std::vector<point_t>& B_array, const ml2s_signature& sig, const std::vector<point_t>& I_array)
{
  ...
  auto points_array_to_dict = [](const std::vector<point_t>& v) -> std::string
  {
    ...
    size_t idx = 1;
    for (auto& p : v)
      s += num_to_string_fast(idx++) + ": ed25519.Point('" + p.to_string() + "'), ";
    if (s.size() > 2)
  ...
  auto scalars_array_to_dict = [](const std::vector<scalar_t>& v) -> std::string
  {
    ...
    size_t idx = 1;
    for (auto& p : v)
      s += num_to_string_fast(idx++) + ": 0x" + p.to_string_as_hex_number() + ", ";
    if (s.size() > 2)
      s.erase(s.end() - 2, s.end());
    return s;
  };
...
TEST(ml2s, cpp2py)
{
  ...
  save_string_to_file("ml2s_sig_check_test.py", str);
...
TEST(ml2s, sig_verif_performance)
{
  ...
  for (size_t p = 0; p < tests_cnt; ++p)
  {
    ...
    LOG_PRINT_L0(" stats over " << std::dec << p + 1 << " runs:");
...
int crypto_tests(const std::string& cmd_line_param)
{
  ...
  for (size_t i = 0; i < g_tests.size(); ++i)
  {
    ...
    LOG_PRINT("   " << std::setfill(' ') << std::setw(40) << std::left << test.first << " >", LOG_LEVEL_0);
